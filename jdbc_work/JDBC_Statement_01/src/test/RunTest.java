...
public class RunTest {
	...
	public static void main(String[] args) {
		...
		// 1. 각자 pc(localhost)에 JDBC계정에 연결한 후에 TEST테이블에 insert 해보기
		// insert => 처리된 행 수(int값)으로 반환 => 트랜잭션 처리
		
		
		// 필요한 변수 먼저 세팅
		int result = 0;  //결과(처리된 행 수)를 받아줄 변수
		Connection conn = null; //DB의 연결정보를 보관할 객체 // import 시키기
		Statement stmt = null;  // SQL문 전달해서 실행 후 결과를 받는 객체
		
		// 앞으로 실행할 sql문 ("완성형태"로 만들어 두기)(맨 뒤의 세미콜론은 작성X)
		...
		try {
			...
			Class.forName("oracle.jdbc.driver.OracleDriver");
				// ojdbc6.jar 파일을 추가 안했거나, 추가는 했으나 오타가 있을 경우 => ClassNotFoundException 오류발생
			// System.out.println("OracleDriver 등록성공");
			
			// 2) Connection 객체 생성 : DB연결 (url, 계정명, 비밀번호)
			conn = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "JDBC", "JDBC");
			
			// 3) Statement 객체 생성
			stmt = conn.createStatement();
			
			// 4, 5) SQL문 전달하고 실행 후 결과받기 (처리된 행 수)
			result =stmt.executeUpdate(sql);
			// 실행할 sql문이 dml문(insert, update, delete)일 경우 stmt.executeUpdate("dml문"); => int
			// 실행할 sql이 select문일 경우 stmt.eqcuteQuery("select문"); => resultSet
		...
		*/
		...
		// 2. 내 pc DB상의 jdbc계정에 test테이블의 모든 데이터 조회해보기
		// SELECT문 => 결과 resultSet (조회된 데이터들이 담겨있음) 받기 => resultSet 으로부터 데이터 뽑기
		...
		try {
			...
			// rset.next() => rset의 다음행이 들어있는지 없는지 확인후 결과반환(boolean값) + 한행을 내려줌
			while(rset.next()) {
				// 현재 참조하는 rset으로부터 어떤 컬럼에 해당하는 값을 어떤 타입으로 뽑을건지 제시해주어야 함
				// DB의 컬럼명을 제시 (대소문자를 가리지 않는다)
